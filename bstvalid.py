...
class Node:
    ...
    def is_valid(self):
        ...
        def check(self, less_than, greater_than):

            if self is None:
                return False

            if less_than is not None and self.left < less_than:
                return False

            if greater_than is not None and self.right > greater_than:
                return False

            if not check(less_than, self, greater_than):
                return False

            if not check(less_than, greater_than, self):
                return False
